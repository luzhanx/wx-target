...
const api = {
   ...
   * @param {*点赞用户id} id
   */
  probability: (id) => http.get('/index/probability', {
    id: id
  }),
   ...
   * @param {*用户id} uid
   */
  probabilityPage: (uid) => http.get('/index/probabilityPage', {
    id: uid
  }),
   ...
   * @param {*用户id} uid
   */
  targetDetails: (uid) => http.get('/index/targetDetails', {
    id: uid
  }),
   ...
   * @param {*目标id} uid
   */
  templateDetails: (id) => http.get('/template/details', {
    id: id
  }),
  /**
   * 菊花码分享页面
   * @param {*用户ID} id
   */
  chrysanthemum: (id) => http.get('/chrysanthemum/index', {
    id: id
  }),
  /**
   * 新建目标
   ...
   * @param {*数据} data
   */
  addTarget: (file, data) => http.uploadFile('/index/addTarget', {
    path: [file],
    name: 'images'
  }, data),
   ...
   * @param {*目标ID} id
   */
  delTarget: (id) => http.get('/index/delTarget', {
    id: id
  }),
  /**
   * 编辑提醒页面
   * @param {*目标ID} id
   */
  GeteditTargetPage: (id) => http.get('/index/editTargetPage', {
    id: id
  }),
   ...
   */
  dataImages: (data) => {
    return http.uploadFile('/data/images', {
      name: data.name,
      path: [data.path]
    }, {});
  },
