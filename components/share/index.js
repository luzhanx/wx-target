   ...
   */
     ...
     */
			...
			wx.getImageInfo({
				...
				success(res) {
					...
					that.setData(
						{
							avatarPath: res.path,
							sharePicPath: './../../assets/img/share.png'
						},
						() => {
							that.drawImage();
						}
		...
		drawImage: function() {
			...
			ctx.beginPath();
			ctx.setStrokeStyle('#fff');
			ctx.moveTo(windowWidth * 0.31, windowHeight * 0.4);
			ctx.lineTo(windowWidth * 0.68, windowHeight * 0.4);
			ctx.stroke();
			...
			// 绘制菊花码
			ctx.drawImage(that.data.jhm, windowWidth * 0.63, footerY * 1.1, 0.3 * windowWidth, 0.3 * windowWidth);
			ctx.save();
