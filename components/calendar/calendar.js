...
Component({
  ...
  methods: {
    ...
    _setCalendarData: function (year, month) {
      const empty_days_count = new Date(year, month - 1, 0).getDay(); // 本月第一天是周几，0是星期日，6是星期六
      ...
      const last_date = last_day.getDay()-1;                // 本月最后一天是星期几
      ...
      for (let i = 0; i < this.data.days_color.length; i++) {
        ...
        for (let j = 0; j < days.length; j++) {
          if (days[j].info == item.month && days[j].day == item.day) {
            ...
            if (item.border) {
              days[j].border = item.border + '!important';
            }
          }
        }
      }
